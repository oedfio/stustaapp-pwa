...
The `stustaapp` systemd service runs uvicorn with multiple worker processes (observed as 3 separate PIDs in production logs) rather than a single process. Each worker is a fully separate Python process with its own in-process APScheduler instance — see [§7's note on duplicate pushes](#duplicate-pushes-across-worker-processes) for why that matters for scheduled jobs specifically.
...
4. The service worker (`frontend/src/sw.js`) handles the `push` event (shows the OS notification) and `notificationclick` (focuses/opens the app to the relevant URL). `showNotification` intentionally omits `icon` — Android already renders its own small app-identity avatar next to the notification text (pulled from the manifest icon), so passing a matching `icon` just draws a second, near-identical thumbnail beside it. It does pass `badge` (`frontend/public/notification-badge.png`, a white bell silhouette on a transparent background): Android renders `badge` in the status bar using only its alpha channel, so it has to be a monochrome/transparent asset — pointing it at a full-color, mostly-opaque icon (as it originally was) renders as a solid blob there. iOS Safari ignores `badge` entirely and always shows the installed app's icon.

### Duplicate pushes across worker processes

Production runs uvicorn with multiple worker processes (see [§5 Server Infrastructure](#5-server-infrastructure)), and each worker has its own in-process `AsyncIOScheduler` (`app/main.py`) — without any coordination, every worker's scheduler fires `send_event_reminder_notifications`/`cleanup_unused_media` at the same tick, so a single logical event produced one duplicate push per worker. `app/tasks.py`'s `run_if_unlocked(lock_key, ttl_seconds, job)` guards against this: it does a Redis `SET lock_key 1 NX EX ttl_seconds` before running a job, so only the worker that wins the race actually executes it — the rest see the key is already set and skip that tick. The scheduler registers the locked wrappers (`cleanup_unused_media_locked`, `send_event_reminder_notifications_locked` in `app/main.py`), not the underlying functions directly. TTLs are short relative to each job's interval (120s for the 5-minute reminder job, 30 minutes for the weekly cleanup) — just long enough to cover the multi-worker burst at a given tick, not so long they'd ever block that job's next legitimate run.
